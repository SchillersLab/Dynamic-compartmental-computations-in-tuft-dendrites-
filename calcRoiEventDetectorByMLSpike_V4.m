function [SpikeTrainStart, SpikeTrainEnd, SpikeTrainPks,SpikeTrainH, eventDetector_activity, comb_activity] = calcRoiEventDetectorByMLSpike_V4(dataCurROI, ImageSampleTime, frameNum, aV, outputpath, activityIndex, clusterCount, roiName, sigmaChangeValue, roiCount)       
    traceSig            = dataCurROI;
...
% -------------------------------------------------------------------------------    
    ...
    for i = 1:length(countVecFix)
        ...
        end_find_res = find(fit(afterS:nextPos) <= drift(afterS:nextPos) + 0.25 * drift_std, 1);
